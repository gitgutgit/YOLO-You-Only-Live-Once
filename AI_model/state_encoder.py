...
def encode_state(detections: list, game_state: dict = None) -> np.ndarray:
    ...
    if game_state is not None and "lava" in game_state:
        lava = game_state["lava"]
        player_x = vec[0]

        # warning / active 플래그는 game_state에서 직접 가져옴
        if lava["state"] == "warning":
            vec[20] = 1.0
        elif lava["state"] == "active":
            vec[21] = 1.0

        # 위치도 YOLO 말고 game_state로 써도 됨 (더 정확)
        lava_center_x = (lava["zone_x"] + lava["zone_width"] / 2) / WIDTH
        vec[22] = np.clip(lava_center_x - player_x, -1, 1)
